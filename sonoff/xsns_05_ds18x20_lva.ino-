/*
  xsns_05_ds18x20_lva.ino - DS18x20 temperature sensor support for Sonoff-Tasmota
...
struct DSBUSES { // lva
    uint8_t pin;
    uint8_t sensors; // сенсоров на шине
} ds_buses[DS_BUSES];

ds_buses[0].pin = 0;
ds_buses[1].pin = 0;
ds_buses[0].sensors=0;
ds_buses[1].sensors = 0;
...
uint8_t OneWireReset(uint8_t ds18x20_pin) { // updated
  uint8_t retries = 125;
...
void OneWireWriteBit(uint8_t ds18x20_pin, uint8_t v) { // updated
...
uint8_t OneWireReadBit(uint8_t ds18x20_pin) { // updated
  //noInterrupts();
  pinMode(ds18x20_pin, OUTPUT);
  digitalWrite(ds18x20_pin, LOW);
  delayMicroseconds(3);
...
void OneWireWrite(uint8_t ds18x20_pin, uint8_t v) {  // updated
  for (uint8_t bit_mask = 0x01; bit_mask; bit_mask <<= 1) {
    OneWireWriteBit( ds18x20_pin, (bit_mask & v) ? 1 : 0);
  }
}

uint8_t OneWireRead(uint8_t ds18x20_pin) { // updated
  uint8_t r = 0;

  for (uint8_t bit_mask = 0x01; bit_mask; bit_mask <<= 1) {
    if (OneWireReadBit(ds18x20_pin)) {
      r |= bit_mask;
    }
  }
  return r;
...
void OneWireSelect(uint8_t ds18x20_pin, const uint8_t rom[8]) { //updated
  OneWireWrite(ds18x20_pin, W1_MATCH_ROM);
  for (uint8_t i = 0; i < 8; i++) {
    OneWireWrite(ds18x20_pin, rom[i]);
  }
}
...
uint8_t OneWireSearch(uint8_t ds18x20_pin, uint8_t *newAddr) {
  ...
  if (!onewire_last_device_flag) {
    if (!OneWireReset(ds18x20_pin)) {
      onewire_last_discrepancy = 0;
      onewire_last_device_flag = false;
      onewire_last_family_discrepancy = 0;
      return false;
    }
    OneWireWrite(ds18x20_pin, W1_SEARCH_ROM);
    do {
      id_bit     = OneWireReadBit(ds18x20_pin);
      cmp_id_bit = OneWireReadBit(ds18x20_pin);
      ...
      if ((id_bit == 1) && (cmp_id_bit == 1)) {
        ...
        OneWireWriteBit(ds18x20_pin, search_direction);
        id_bit_number++;
        rom_byte_mask <<= 1;
        if (rom_byte_mask == 0) {
          rom_byte_number++;
...
void Ds18x20Init()
{
  ...
  ds_buses[0].pin = pin[GPIO_DSB];
  ds_buses[1].pin = pin[GPIO_DSB2];

  OneWireResetSearch(); // без шины

  uint8_t pre_sensors=0;
  for (uint8_t bus = 0; bus < DS_BUSES; bus++) {
      if (ds_buses[bus].pin < 99) { // т.е.  если он назначен
        for (uint8_t bus_sensor = 0; ds18x20_sensors < DS18X20_MAX_SENSORS; bus_sensor)  {
            ds18x20_sensors = bus_sensor + pre_sensors;
            if (!OneWireSearch(ds_buses[bus].pin, ds18x20_sensor[ds18x20_sensors].address)) {
                break;
            }
            if (OneWireCrc8(ds18x20_sensor[ds18x20_sensors].address) &&
                ...
                (ds18x20_sensor[ds18x20_sensors].address[0] == MAX31850_CHIPID))) {
                  ...
                  ds_buses[bus].sensors = bus_sensor;
                  bus_sensor++;
              }
        }
        snprintf_P(log_data, sizeof(log_data), PSTR(D_LOG_DSB "Bus:%d sensors found:%d\n"), bus, ds_buses[bus].sensors);
      }
      pre_sensors = pre_sensors + ds_buses[bus].sensors; 
  }
  for (uint8_t i = 0; i < ds18x20_sensors; i++) { // тупо соритровка
...
void Ds18x20Convert(uint8_t ds18x20_pin) {// updated
  OneWireReset(ds18x20_pin);
#ifdef W1_PARASITE_POWER
...
#else
  OneWireWrite(ds18x20_pin, W1_SKIP_ROM); // Address all Sensors on Bus
#endif
  OneWireWrite(ds18x20_pin, W1_CONVERT_TEMP); // start conversion, no parasite power on at the end
...
bool Ds18x20Read(uint8_t sensor)
{
  ...
  for (uint8_t retry = 0; retry < 3; retry++) {
    OneWireReset(ds18x20_sensor[index].pin);
    OneWireSelect(ds18x20_sensor[index].pin, ds18x20_sensor[index].address);
    OneWireWrite(ds18x20_sensor[index].pin, W1_READ_SCRATCHPAD);
    for (uint8_t i = 0; i < 9; i++) {
      data[i] = OneWireRead(ds18x20_sensor[index].pin);
    }
    if (OneWireCrc8(data)) {
      ...
      case DS18B20_CHIPID:
        if (data[4] != 0x7F) {
          ...
          OneWireReset(ds18x20_sensor[index].pin);
          OneWireSelect(ds18x20_sensor[index].pin, ds18x20_sensor[index].address);
          OneWireWrite(ds18x20_sensor[index].pin, W1_WRITE_SCRATCHPAD);
          OneWireWrite(ds18x20_sensor[index].pin, data[2]);    // Th Register
          OneWireWrite(ds18x20_sensor[index].pin, data[3]);    // Tl Register
          OneWireWrite(ds18x20_sensor[index].pin, data[4]);    // Configuration Register
          OneWireSelect(ds18x20_sensor[index].pin, ds18x20_sensor[index].address);
          OneWireWrite(ds18x20_sensor[index].pin, W1_WRITE_EEPROM); // Save scratchpad to EEPROM
#ifdef W1_PARASITE_POWER
